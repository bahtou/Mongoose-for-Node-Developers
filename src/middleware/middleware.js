...
module.exports.validateSignup = function(req, res, next) {
  console.log("validate signup begin");

  // first check that requirements are not empty
  req.validate('username', 'Username is required').notNull();
  req.validate('password', 'Password is required').notNull();
  req.validate('verify', 'Password must match').notNull();
  ...
  // second, check that requirements pass format
...
module.exports.validateLogin = function(req, res, next) {
  ...
  if (req.validationErrors(true)) {
    ...
    return res.render('login', {
      title: 'Login Error',
      username: req.body.username,
      loginError: req.validationErrors(true)
    });
  }
  ...
  User.findOne({_id: req.body.username}, function(err, user) {
    ...
    if (!new User().authenticate(user.password, req.body.password, salt)) {
      console.log('user password is not a match');
      return res.render('login', {
        title: 'Login Error',
        username: req.body.username,
        loginError: {password: {msg: 'Invalid username or password'}}
      });
    }
